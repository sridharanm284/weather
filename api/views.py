...
class ForecastAPI(generics.ListCreateAPIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({'headers': datas.total_headers, 'datas': datas.ScrapeDatas(),'interval': datas.interval})
        except:
            return Response({'interval': [],'headers': [], 'datas': []})
...
def login(request):
    ...
    try:
        ...
        if userdata['password'] == password:
            id1=userdata['id']
            return Response(createtoken(id1,name,userdata), status=status.HTTP_200_OK)
        else:
...
def sendMessage(request):
    ...
    if files:
        for file in files:
            msg=Chat(chat_id=chat_id, message=message, user_name=user, file=file)
            msg.save(using="observation")
    else:
        msg=Chat(chat_id=chat_id,message=message,user_name=user)
        msg.save(using="observation")
    if user=="user":
...
def ChatsList(request):
    user=request.data.get('user')
    get_user_id=Tokens.objects.using("observation").values().get(token=user)
...
def stormDatas(request):
    ...
    try:
        ...
        cur1.execute(f"SELECT * FROM tbl_storm_path WHERE storm_track_id in ({', '.join({str(x.get('storm_track_id')) for x in storm_track_datas})}) ORDER BY storm_date DESC")
        storm_path = cur1.fetchall()
        storm_path_dict = {f"storm_{x.get('storm_track_id')}": None for x in storm_path}
        for x in storm_path:
            if storm_path_dict.get(f"storm_{x.get('storm_track_id')}") != None:
                continue
            storm_path_dict[f"storm_{x.get('storm_track_id')}"] = x
        storm_path = [x for x in storm_path if x.get('storm_date').day == storm_path[0].get('storm_date').day]
        storm_path_datas = {x.get("storm_path_id"):x for x in storm_path}
        tbl_storm_path_array = []
        for x, y in storm_path_datas.items():
            cur1.execute(f"SELECT * FROM tbl_storm_path_data WHERE storm_path_id={str(x)}")
            datas = cur1.fetchall()
            if (y.get('storm_path_id') == storm_path_dict.get(f"storm_{y.get('storm_track_id')}").get('storm_path_id')):
                storm_path_dict[f"storm_{y.get('storm_track_id')}"] = {"lat": datas[0].get('lat'), 'lon': datas[0].get('lon')}
...
def send_email(request):
    subject = request.data.get('subject')  # Get the subject from the request
    message = request.data.get('message')  # Get the message from the request
    recipient_email = request.data.get('recipient_email')  # Get the recipient's email from the request

    # Define the sender's email address (change to your own email)
    ...
    try:
        ...
        return Response({'status': 'Email sending failed', 'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
