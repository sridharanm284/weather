...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if text_data_json["mode"] == "createchat":
            ...
            cursor.execute(f"SELECT * FROM api_chatrooms WHERE user_id={user_id}")
            room = json.loads(json.dumps(cursor.fetchone(), cls=DateTimeEncoder))
            cursor.execute(f"SELECT * FROM api_chat WHERE chat_id='{chat_id}' ORDER BY date_time DESC LIMIT 1")
            chats = json.loads(json.dumps(cursor.fetchone(), cls=DateTimeEncoder))
            print(room)
            print(chats)
            data = {
                    'rooms': room,
            ...
            try:
                cursor.execute(f"SELECT * FROM api_chatrooms WHERE user_id={user_id}")
                room = json.loads(json.dumps(cursor.fetchone(), cls=DateTimeEncoder))
                cursor.execute(f"SELECT * FROM api_chat WHERE chat_id='{room.get('chat_id')}' ORDER BY date_time DESC LIMIT 1")
                chats = json.loads(json.dumps(cursor.fetchone(), cls=DateTimeEncoder))
