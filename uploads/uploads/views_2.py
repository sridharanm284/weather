...
def createtoken(user_id,name,data):
    ...
    t=jsonwebtoken.encode(payload,"WeatherApp","HS256")
...
def sendMessage(request):
    ...
    if files:
        for file in files:
            msg=Chat(chat_id=chat_id, message=message,date_time='',user_type=user, file=file)
...
def serve_image(request, file_path):
    # Construct the absolute file path based on your MEDIA_ROOT
